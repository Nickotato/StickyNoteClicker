import { defaultWorkers, defaultUpgrades, defaultAchievements, defaultVisuals } from "../defaults/defaults.mjs";
import {findUpgradeCost} from "../utils.mjs"
import {addAchievementNote, updateAchievementStats} from "../achivementsLogic.mjs";
import {updateShopDescriptions } from "../shop.mjs";
...
function save(game) {
  ...
  function load(game, saveSlot = "save1") {
    ...
    if (savedGame.upgrades) {
      Object.keys(defaultUpgrades).forEach((key) => {
        ...
        const cost = Math.floor(baseCost * Math.pow(findUpgradeCost(key), owned));
    ...
    if (savedGame.lastOnline) {
      ...
      if (secondsOffline > 60) {
        ...
        const multiplier = (game.upgrades.upgrade4?.owned || 0) * (game.upgrades.upgrade4?.value || 1);
        const offlineEarnings = secondsOffline * moneyPerSecond * multiplier;
        if (offlineEarnings > 0) {
          game.money += offlineEarnings;
          alert(`You were away for ${secondsOffline} seconds and earned ${readableNumber(offlineEarnings)}!`);
        }
      }
    }
  ...
  function newGame(game, saveSlot = "save1", confirmReset = true) {
    if (
      confirmReset &&
      !confirm("Are you sure you want to start a new game? This will delete your current progress!")
    ) {
      return;
    }
